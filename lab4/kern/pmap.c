...
boot_alloc(uint32_t n)
{
	...
	if(n>0)
	{
		 // incrementing the nextfree upto the requested number of bytes
...
//	panic("x64_vm_init: this function is not finished\n");
	...
	boot_map_region(pml4e,UPAGES,PTSIZE,PADDR(pages),(PTE_U));
...
mem_init_mp(void)
{
	...
	for(i=0;i<NCPU;i++)
	{
		boot_map_region(boot_pml4e,(KSTACKTOP-((i*(KSTKSIZE+KSTKGAP))))-KSTKSIZE,KSTKSIZE,PADDR(percpu_kstacks[i]),(PTE_W));
	}
...
page_init(void)
{
	...
	// leaving the IO hole and Boot Loader allocation sections
...
page_free(struct PageInfo *pp)
{
	...
	{
		panic("panic: This is not a free page\n");
	}
...
pml4e_walk(pml4e_t *pml4e, const void *va, int create)
{
	...
	if(!(*pml4e & PTE_P))
	{
		...
		else
		{
			...
			new_page->pp_ref++;
			pdpe = (pdpe_t *)page2kva(new_page);   
			pt= pdpe_walk(pdpe,va,create);
			if(!pt)
			{
			...
			{
				*pml4e = PADDR(pdpe)|PTE_P|PTE_W|PTE_U;
			}	
		}
	 }
	 else
	 {
			 pdpe = KADDR(PTE_ADDR(*pml4e));
			 pt= pdpe_walk(pdpe,va,create);
			 return pt;		
	 }	
	 return pt;
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create){
	...
	//index
	size_t index=PDPE(va);
        pte_t *pgdir,*pt;
        //pdpe_t pdp  = pdpe[index];
	pdpe = &pdpe[index]; 
        struct PageInfo* new_page;
        //check whether Pgdir exists or not
        if(!(*pdpe & PTE_P))
        {
                if (!create)
                        return NULL;
                else
		{
			  ...
			  if(!new_page)
                        ...
                        new_page->pp_ref++;
                //        pdpe[index]= page2pa(new_page) | PTE_U | PTE_W | PTE_P;
                        pgdir = (pde_t *)page2kva(new_page);
			 pt = pgdir_walk(pgdir,va,create);
        	    	if(!pt)
			{
	   	            	page_decref(new_page);
				*pdpe=0;
			...
			{
		  		*pdpe = PADDR(pgdir) | PTE_U | PTE_W | PTE_P;	
			}	   
		}
	 }
	 else
         {
                         pgdir = KADDR(PTE_ADDR(*pdpe));
			 pt = pgdir_walk(pgdir,va,create);
         }
	 return pt;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	 //index
        ...
        pgdir  = &pgdir[index];
        struct PageInfo* new_page;
        //check whether PTE exists or not
        if(!(*pgdir & PTE_P))
        {
                if (!create)
                        return NULL;
                else
		{
                        ...
                        new_page->pp_ref++;
                       *pgdir= PTE_ADDR(page2pa(new_page)) | PTE_U | PTE_W | PTE_P;
                        pt = (pte_t *)page2kva(new_page);
         	}
	 }
         else
         {
                        pt = KADDR(PTE_ADDR(*pgdir));
         }
         size_t pt_index =  PTX(va);
         return &pt[pt_index];
}
...
//	cprintf("Size %x\n",size);
	pte_t* pt;
	perm = perm | PTE_P;
	// assuming size is a multiple of PGSIZW 
	while(size>=PGSIZE)
	{
		pt=pml4e_walk(pml4e,(const void *)la,perm);
...
//	cprintf("page insert %x\n %d",pte,E_NO_MEM);
	...
	if(*pte & PTE_P)
	{
		//if the same exists then simply update the permission bits
		//if(PTE_ADDR(*pte) == pa)
		//	goto samePage;
		// removing the older page			
...
{
	// Fill this function in
	pte_t* pt=pml4e_walk(pml4e,va,0);
	physaddr_t pa;
	if(!pt)
	{
		return NULL;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	// LAB 3: Your code here.
	perm = perm | PTE_P;
	pte_t* pml4e = env->env_pml4e;
	pte_t* pt;
	int i=0,count=0;
	uintptr_t va_check = (uintptr_t)va;
	uintptr_t va_new = (uintptr_t)va;
	uintptr_t va_end = ROUNDUP((va_new + len),PGSIZE);
		
	while(va_new < va_end )
	{
		
		if((uint64_t)(va_new)>=ULIM)
		{	
			user_mem_check_addr = va_new;
			return -E_FAULT;
		}
		 
		pt = pml4e_walk(env->env_pml4e,(void *)(va_new),0);
		if(!pt || !(*pt & perm))
		{
			cprintf("inside memcheck\n");	
			user_mem_check_addr = va_new;
                        return -E_FAULT;
		}
		...
		if(count == 0)
		{
			va_check = (uintptr_t)ROUNDUP(va_new, PGSIZE);
			va_new = va_check;
			count++;	
		}
		else
		{
			va_new = va_new + PGSIZE;
		}
...
check_page_free_list(bool only_low_memory)
{
	...
	cprintf("check page free list succeded\n");
}
...
check_boot_pml4e(pml4e_t *pml4e)
{
	...
	//	cprintf("check_boot %x %x\n",UPAGES+i,pml4e);
	//	check_va2pa(pml4e,UPAGES);
