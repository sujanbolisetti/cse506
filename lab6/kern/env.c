...
env_setup_vm(struct Env *e)
{
	...
	//struct PageInfo* pml4ePage = page_alloc(0);
	memset(page2kva(p),0,PGSIZE);
	e->env_pml4e = (pml4e_t *)page2kva(p);
	e->env_cr3=page2pa(p);
	//e->env_pml4e = (pte_t *) boot_pml4e[PML(UTOP)];
	//cprintf("pml4 %x\n",PML4(UTOP));
	e->env_pml4e[1]= boot_pml4e[1];
	//memset(e->env_pml4e, 0, PML4(UTOP)*sizeof(pte_t));
	p->pp_ref++;
...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	//cprintf(" before generation %d %d\n",e->env_id,e->env_type);
	generation = (e->env_id + (1 << ENVGENSHIFT)) & ~(NENV - 1);
	if (generation <= 0)	// Don't create a negative env_id.
		generation = 1 << ENVGENSHIFT;
	//cprintf(" env_Id as generation %d %d %d\n",generation,e,envs);
	e->env_id = generation | (e - envs);
	
	//cprintf("inside env_alloc %d %d \n",e->env_id,generation);
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	int i=0;
	struct PageInfo *pp;
	//size_t roundedLen =  ROUNDUP(len+PGOFF(va),PGSIZE);
	for(i=0;i<len;i+=PGSIZE)
	{
		...
		if(!pp)
			...
			page_insert(e->env_pml4e,pp,va+i,PTE_U | PTE_P | PTE_W);
			//roundedVa+=PGSIZE;
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	struct Elf* el = (struct Elf*)binary;
	e->elf = binary;
	if(el->e_magic != ELF_MAGIC)
		panic("Bad Elf Header");
	
	lcr3(PADDR(e->env_pml4e));
	
	struct Proghdr* ph;
	ph = (struct Proghdr *)((uint8_t *)el + el->e_phoff);
	struct Proghdr* eph = ph + el->e_phnum; 	
	//cprintf("offset %x\n",el->e_phnum);
	// we are looping until we cover all the program headers
	while(ph<eph)
	{
		if(ph->p_type == ELF_PROG_LOAD)
		{	// only if the file size is less than memsz we will allocate 
			if(ph->p_filesz <= ph->p_memsz)
			{
				region_alloc(e, (void *)ph->p_va, ph->p_memsz);
				memcpy((void *)ph->p_va, binary + ph->p_offset, ph->p_filesz);
				// setting the extra memory to zeros
				memset((void *)(ph->p_va + ph->p_filesz), 0, (ph->p_memsz - ph->p_filesz));
			}
			else
				panic("size of program header is greater than the memsize");
		}
		ph++;
	
	}
	lcr3(boot_cr3);
	struct PageInfo *pgSta = page_alloc(0);
	if(!pgSta)
		panic("Out Of Memory no pages are available\n");
	
	page_insert(e->env_pml4e,pgSta,(void *)USTACKTOP-PGSIZE,PTE_U | PTE_W);
	e->env_tf.tf_rip = el->e_entry;
       // e->elf = binary;
}
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	if(type == ENV_TYPE_NS)
	{
		//cprintf("I am network server env %d %d",ENV_TYPE_NS);
	}
	//cprintf("created ENV %d %d %d\n",env->env_id,env->env_type,ENV_TYPE_NS);
}

//
// Frees env e and all memory it uses.e
//
	void
