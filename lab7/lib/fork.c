...
duppage(envid_t envid, unsigned pn)
{
	...
	// doing And operation will give us the offset of the physical page from va
	if((*pgtentry) & (PTE_SHARE))
	{
		pageOffset = ((*pgtentry)) & PTE_SYSCALL;
					...
					if((uvpt[VPN(va)] & PTE_P))
					{
						...
						else
						{
							...
							pte_t* pgtentry = (pte_t *)&(uvpt[VPN(va)]);
                                                	// doing And operation will give us the offset of the physical page from va
                                                	pageOffset = ((*pgtentry)) & PTE_SYSCALL;
                                                	res = sys_page_map(0, (void *)va, childEnvid, (void *)va, pageOffset);
                                                	if(res < 0)
                                                        	panic("Error in sharing the page\n");
						}
					}
					va+=PGSIZE;
                                }
