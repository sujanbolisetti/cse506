...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	int r;
        struct OpenFile *op;
        r=openfile_lookup(envid,req->req_fileid,&op);
        if(r<0)
        {
                return r;
        }
        struct File *file = op->o_file;
        struct Fd *fd = op->o_fd;
        size_t actualReadCount = req->req_n;
        //cprintf("%d\n %d %d\n",req->req_n,actualReadCount,PGSIZE);
        if(req->req_n >= PGSIZE)
        {
                actualReadCount = PGSIZE;
        }
        int pos = file_read(file,ret->ret_buf,actualReadCount,fd->fd_offset);
        if(pos<0)
                return pos;
