...
send_data(struct http_request *req, int fd)
{
	...
	int r;
	struct Fd *filedes;
        if((r = fd_lookup(fd,&filedes)) < 0)
                return r;

	int len = strlen(fd2data(filedes));
	if(write(req->sock, fd2data(filedes),len) != len){
		return -1;
	}*/
	int r;
	char buff[64];
...
send_file(struct http_request *req)
{
	...
	//struct OpenFile *op;
        //r=openfile_lookup(thisenv->env_id,fd->fd_file.id,&op);
        //if(r<0)
        //{
               // return r;
        //}
